...
class BuildFix{
    ...
    public static function Bfix($sid)
    {
		...
		$old_verid = Db::name('sfdp_design_ver')->where('sid',$info['id'])->where('status',1)->value('id');
		/*新的设计写入版本库*/
		...
		foreach($varInfo['db'] as $k=>$v){
			...
			if($fiels){
				...
				//新增字段信息
				if($v['tpfd_dblx']=='datetime'||$v['tpfd_dblx']=='longtext'||$v['tpfd_dblx']=='date'){
					$sql[] = "ALTER TABLE {$table} ADD COLUMN {$v['tpfd_db']} {$v['tpfd_dblx']} COMMENT '{$v['tpfd_name']}';";
				}else{
					$sql[] = "ALTER TABLE {$table} ADD COLUMN {$v['tpfd_db']} {$v['tpfd_dblx']}({$v['tpfd_dbcd']}) NOT NULL DEFAULT '' COMMENT '{$v['tpfd_name']}';";
				}
        ...
        try {
            ...
            if(count($sql)>0){
                /*如果数据表有变更，创建一个备份表*/
                Db::query("create table {$table}_bak like {$table};");
                Db::query("INSERT INTO {$table}_bak SELECT * FROM {$table};");
                foreach($sql as $v){
                    Db::query($v);
                }
            }
